...
public class Authenticator implements okhttp3.Authenticator
{
    ...
    public boolean invalidUsernameOrPassword=false;
    private int authenticationRequestsCount=0;

    public Authenticator(@NonNull String login, @NonNull String password) {
        this(login, password, "", "");
    }

    public Authenticator(@NonNull String login, @NonNull String password, @NonNull String domain, @NonNull String workstation) {
        mLogin = login;
        mPassword = password;
        mDomain = domain;
    ...
    public Request authenticate(Route route, Response response) throws IOException {
        ...
        authenticationRequestsCount++;
        if (authenticationRequestsCount>=3&&response.code()==401)
        {
